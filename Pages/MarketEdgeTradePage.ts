...
export class MarketEdgeTradePage {
  readonly page: Page;
  ...
  async selectMarketType(marketType: string): Promise<void> {
    ...
    await this.page.locator("//span[normalize-space()='Market-Edge']").click();
    await this.page.waitForTimeout(1000);
  }
  ...
  async performTrade(tradeDetails: {
    ...
    const { exchange, marketType, symbol, quantity, duration, decay, side } = tradeDetails;
