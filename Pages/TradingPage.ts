...
export class TradingPage {
  ...
  constructor(page: Page) {
    ...
    this.symbolInput = page.locator("//div[@data-testid='symbol-option-DOT-EUR']");
  ...
  async selectExchange(exchangeName: string): Promise<void> {
    ...
    await this.exchangeSearchInput.waitFor({ state: 'visible', timeout: 50000 });
    await this.exchangeSearchInput.fill(exchangeName);
    await this.page.waitForTimeout(1000);
    await this.page.locator("//div[@data-testid='account-row-item']").click();
    await this.page.waitForTimeout(1000);
  }
  ...
  async selectMarketType(marketType: string): Promise<void> {
    ...
    await this.page.waitForTimeout(1000);
    await this.page.locator("//div[@data-testid='GOTRADE_ORDERTYPE_MARKET']").click();
    await this.page.waitForTimeout(1000);
  }
  ...
  async performTrade(tradeDetails: {
    ...
    const { exchange, marketType, symbol, quantity, side } = tradeDetails;
