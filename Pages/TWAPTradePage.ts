...
export class TWAPTradePage {
  ...
  async selectMarketType(marketType: string): Promise<void> {
    ...
    await this.page.waitForTimeout(1000);
    await this.page.locator("//div[@data-testid='GOTRADE_ORDERTYPE_TWAP']").click();
    await this.page.waitForTimeout(1000);
  }
  ...
  async performTrade(tradeDetails: {
    ...
    const { exchange, marketType, symbol, quantity, duration, interval, side } = tradeDetails;
