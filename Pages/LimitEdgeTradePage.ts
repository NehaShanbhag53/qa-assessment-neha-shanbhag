...
export class LimitEdgeTradePage {
  readonly page: Page;
  ...
  constructor(page: Page) {
    ...
    this.symbolInput = page.locator("//div[@data-testid='symbol-option-BTC-USDT-SWAP']");
   // this.quantityInput = page.locator('input[id="emailInput"][data-testid="quantity"]');
    ...
    this.quantityInput = page.locator("//input[@data-testid='quantity']");
    this.durationInput = page.locator("//input[@data-testid='duration']");
    this.thresholdInput = page.locator("//input[@data-testid='threshold']");
  }
  ...
  async selectMarketType(marketType: string): Promise<void> {
    ...
    await this.page.locator("//span[normalize-space()='Limit-Edge']").click();
    await this.page.waitForTimeout(1000);
  }
   ...
   * @param threshold 
   */
  async enterThreshold(threshold: string): Promise<void> {
    await this.thresholdInput.fill('');
  ...
  async performTrade(tradeDetails: {
    ...
    const { exchange, marketType, symbol, quantity, duration, price, threshold, side } = tradeDetails;
    ...
    if (price) {
     ...
     if (threshold) {
      console.log(`Entering threshold: ${threshold}`);
      await this.enterThreshold(threshold);
    }
